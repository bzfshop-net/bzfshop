...
class Show extends \Controller\BaseController
{
    ...
    public function ajaxShow($f3)
    {
        ...
        out_display:
        $smarty->display('cart_show_ajaxshow.tpl');
    }
     ...
     * @param $f3
     */
    public function ajaxGoodsCount($f3)
    {
        $cartBasicService = new CartBasicService();
        $goodsCount       = intval($cartBasicService->getCartOrderGoodsNumber());
        if (!isset($goodsCount)) {
            $goodsCount = 0;
        }

        Ajax::header();
        echo Ajax::buildResult(null, null, $goodsCount);
    }
