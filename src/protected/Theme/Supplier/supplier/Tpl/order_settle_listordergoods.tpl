...
{{block name=order_main_body}}
    ...
    <script type="text/javascript">
        ...
        window.bz_set_breadcrumb_status.push({index: 1, text: '结算明细', link: window.location.href});
    </script>
    <!-- 页面主体内容 -->
    <div class="row">
        ...
        <table class="table table-bordered table-hover">
            ...
            {{if isset($orderGoodsArray)}}
                ...
                {{foreach $orderGoodsArray as $orderGoodsItem }}
                    ...
                    {{/if}}
                    <td>
                        <a href="#" onclick="bZF.Order_ListOrder_Detail({{$orderGoodsItem['rec_id']}});return false;">
                    ...
                    <td>{{$orderGoodsItem['memo']|nl2br nofilter}}</td>
                    </tr>
                    <!-- /一个订单 -->
                {{/foreach}}
            {{/if}}
